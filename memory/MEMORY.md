...
## Next Priorities
1. `aeon-update` sync watermark now `8b8d719` on `main` (PR #64 merged 08-24 13:08 UTC, then PR #68 — 25 more upstream commits `b7a909a..8b8d719` — merged 08-25 23:25 UTC; PR #65 egress-audit hardening also merged 08-24). Standing decision for Aaron: **11 conflicts kept local**, unchanged since 08-25 (no aeon-update run since) — notably `skills/heartbeat`/`memory-flush`/`skill-health` SKILL.md vs. upstream's `0776bbb` scorer rewrite; `.github/workflows/aeon.yml` now also worth a manual look (upstream added a link-local/metadata egress block + iron-proxy audit); plus 2 new deferred skills (`rightstack`, `skill-article`) blocked on an `eyebrowlock.json` regen the runner can't do (binary unavailable). Full list in `memory/topics/aeon-update-state.json` → `pending_conflicts`.
