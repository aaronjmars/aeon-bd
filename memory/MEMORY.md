...
## Next Priorities
1. ~~Investigate `aeon-update` never-dispatched~~ **RESOLVED 2026-08-24:** `aeon-update` ran (first real sync), opening **PR #64** — 43 upstream commits `b1d9079..b7a909a`, 79 files applied, 7 conflicts surfaced. **Action for Aaron: review + merge PR #64** to advance the sync watermark to `b7a909a` (on `main` it stays `b1d9079` until merge). PR flags 3 war-room skills (`heartbeat`, `memory-flush`, `skill-health`) that conflict with upstream's `0776bbb` scorer rewrite — decide whether to pull those in. State: `memory/topics/aeon-update-state.json` (on the PR branch).
