...
- OpenAI Codex reportedly uses "aeon-core" / "nathree-aeon" as an internal persistent-agent codename (surfaced via a reverse-engineered instruction-set leak thread, 2026-09-03, @_DMontgomery40 + reply chain) — pure name collision, unrelated to our @aeonframework product. Expect it to keep surfacing in bare-`aeon` keyword/mention searches; filter on @aeonframework / aeonfun/aeon repo context, not the bare word "aeon" (fetch-tweets 2026-09-04 dropped 6 of 11 candidates on this).
