...
Writes:
- `output/articles/capabilities-sweep-${today}.md` — full human-readable proposal table (every non-error run, including `NO_CHANGES`)
...
A handful of skills write `output/articles/*.md` and only that. After all rules run, if the resulting set is `{read_only}` and the body contains the literal `output/articles/${today}.md` or `output/articles/<slug>-` token, leave it as `{read_only}` and append a `note: writes articles only` field on the proposal manifest. The taxonomy doesn't have a `writes_repo_files` value — and shouldn't, per CAPABILITIES.md's "adding a new capability requires a separate PR" rule. The note exists for operator scanning, not for inclusion in the array.
...
`output/articles/capabilities-sweep-${today}.md`: human-readable version of the same data, sorted by `enabled-first then slug`. Three tables: high-confidence auto-applied, low-confidence needs review, already-declared (skipped this run). The first row of each table is a short sentence explaining what it means.
...
git add skills/*/SKILL.md .outputs/capabilities-sweep-proposals.json output/articles/capabilities-sweep-${today}.md
...
Human-readable article: output/articles/capabilities-sweep-${today}.md
...
Article: output/articles/capabilities-sweep-${today}.md
```
...
No third-party API keys. No on-chain reads. No file writes outside `skills/<slug>/SKILL.md` (in-place frontmatter splice), `output/articles/`, `.outputs/`, and `memory/`.
