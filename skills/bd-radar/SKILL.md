...
Emit it with `./notify`: for a single line, `./notify "<text>"`; if the body runs multiple lines, write it to **`/tmp/bd-radar-notify.md`** first, then `./notify -f /tmp/bd-radar-notify.md` (long multi-line argv trips the sandbox — matches the fleet idiom, e.g. `/tmp/shiplog-notify.md`). **Keep the notify body in `/tmp` — never write it under `memory/` or `output/`**: those are committed OKF roots, so a stray notify file there (missing `type:` frontmatter) fails the `ci-okf` validator.
