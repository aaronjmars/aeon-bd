...
- **Self-check**: if heartbeat's own entry shows `last_success` is **>50 hours ago** (or missing), note that heartbeat itself may be unreliable. (At the daily cadence one missed run is ~48h, so this only fires on two consecutive misses.)
...
- `🔴 DEGRADED` — any P0 flag fired (failed skill, stuck skill, consecutive_failures ≥ 3, chronic failures with success_rate < 0.5, heartbeat self-check >50h stale)
